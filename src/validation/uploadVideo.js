...
export const uploadVideoSchema = Yup.object().shape({
  ...
  video: Yup.object().nullable()
    .when("videoURL", {
      is: (value) => !value,
      then: () => Yup.object().required("Required video"),
    })
    .test("fileSize", "Video size must be less than 50MB", (file) => {
      return file ? file.fileSize <= MAX_FILE_SIZE : true
    })
    .test("fileType", "Unsupported video format", (file) => {
      return file ? SUPPORTED_VIDEO_FORMATS.includes(file.mimeType) : true
    }),
  thumbnail: Yup.object().nullable()
    .when("thumbnailURL", {
      is: (value) => !value,
      then: () => Yup.object().required("Required thumbnail image"),
    })
    .test("fileSize", "Image size must be less than 50MB", (file) => {
      return file ? file.fileSize <= MAX_FILE_SIZE : true
    })
    .test("fileType", "Unsupported image format", (file) => {
      return file ? SUPPORTED_IMAGE_FORMATS.includes(file.mimeType) : true
    }),
})